    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(result, function (k, v) {
                ...
                // Convert and accumulate column-wise totals
                const adv = parseFloat(v['Advance']) || 0;
                const d1_7 = parseFloat(v['1_7_Days']) || 0;
                const d8_15 = parseFloat(v['8_15_Days']) || 0;
                const d16_20 = parseFloat(v['16_20_Days']) || 0;
                const d21_35 = parseFloat(v['21_35_Days']) || 0;
                const d36_50 = parseFloat(v['36_50_Days']) || 0;
                const dOver50 = parseFloat(v['over_50_Days']) || 0;
            ...
            rows += `<tr style="font-weight: bold; background-color: #f8f9fa;">
                ...
                <td></td>
                <td>${totalAdvance.toLocaleString('en-US')}</td>
                <td>${total1_7.toLocaleString('en-US')}</td>
                <td>${total8_15.toLocaleString('en-US')}</td>
                <td>${total16_20.toLocaleString('en-US')}</td>
                <td>${total21_35.toLocaleString('en-US')}</td>
                <td>${total36_50.toLocaleString('en-US')}</td>
                <td>${totalOver50.toLocaleString('en-US')}</td>
                <td colspan="2"></td>
            </tr>`;
        ...
        error: function (xhr, status, error) {
            $(tableID).html(`<tr><td colspan="15" class="text-center text-danger">
                Error loading data: ${xhr.responseText || error}.
            </td></tr>`);
        }
    });
