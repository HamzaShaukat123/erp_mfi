											...
											<div class="col-sm-12 col-md-12 mb-3">
												<div class="d-flex justify-content-between align-items-center">
													...
													<label id="balance-label" class="col-form-label text-success">
														Balance: {{ isset($lager_much_all) ? $lager_much_all->sum('Debit') - $lager_much_all->sum('Credit') : 0 }}
													</label>
...
<script>
	...
	$('#account-select').on('change', function () {
        ...
        if (accountId) {
            $.ajax({
                url: "{{ route('purchase2.get-balance') }}",
                type: "GET",
                data: { account_id: accountId },
                success: function (response) {
                    $('#balance-label').text('Balance: ' + Number(response.balance).toLocaleString());
                },
                error: function () {
