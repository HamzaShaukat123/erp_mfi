        ...
        success: function(result) {
            if (result.pur2 && result.pur2.length > 0) {
                $.each(result.pur2, function(k, v) {
                    ...
                    $('#JV2Table').append(generateRow(v['debit_account'], v['amount'], v['remarks'], v['bankname'], v['instrumentnumber'], v['chqdate'], true));
                    index++; // Increment index for the next row

                    // Generate the 2nd row (Credit Account)
                    $('#JV2Table').append(generateRow(v['credit_account'], v['amount'], v['remarks'], v['bankname'], v['instrumentnumber'], v['chqdate'], false));
                    index++; // Increment index for the next row
                });
