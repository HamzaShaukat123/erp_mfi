...
if ($lager_much_op_bal->isEmpty() || $lager_much_all->isEmpty()) {
    return response()->json(['error' => 'No data available for the provided filters.'], 404);
}

// Safely access the first record
$accountName = htmlspecialchars($lager_much_op_bal[0]['ac_name'] ?? 'N/A');
$address = htmlspecialchars($lager_much_op_bal[0]['address'] ?? 'N/A');
$remarks = htmlspecialchars($lager_much_op_bal[0]['remarks'] ?? 'N/A');
...
$opening_bal = $SOD - $SOC;
$balance = $opening_bal; // Starting balance
...
$pdf->SetCreator(PDF_CREATOR);
$pdf->SetAuthor('MFI');
$pdf->SetTitle("General Ledger R - $accountName");
...
$html = "
    <table style=\"border:1px solid #000; width:100%; padding:6px; border-collapse:collapse;\">
        <tr>
            <td style=\"font-size:12px; font-weight:bold; color:#17365D; padding:5px 10px; border-bottom:1px solid #000; width:70%;\">
                Account Name: <span style=\"color:black;\">$accountName</span>
            </td>
            <td style=\"font-size:12px; font-weight:bold; color:#17365D; text-align:left; padding:5px 10px; border-bottom:1px solid #000; border-left:1px solid #000; width:30%;\">
        ...
        <tr>
            <td style=\"font-size:12px; font-weight:bold; color:#17365D; padding:5px 10px; border-bottom:1px solid #000; width:70%;\">
                Remarks: <span style=\"color:black;\">$remarks</span>
            </td>
            <td style=\"font-size:12px; font-weight:bold; color:#17365D; text-align:left; padding:5px 10px; border-bottom:1px solid #000; border-left:1px solid #000; width:30%;\">
...
<tr>
    <th colspan="3" style="text-align: center;font-weight:bold;">+----Opening Balance----+</th>
    <th></th>
    <th></th>
    <th style="text-align: center">' . number_format($opening_bal, 0) . '</th>
</tr>';
...
foreach ($lager_much_all as $count => $item) {
    $bgColor = ($count % 2 === 0) ? '#f1f1f1' : '#ffffff';
    $debit = $item['Debit'] ?? 0;
    ...
    <tr style=\"background-color:$bgColor;\">
        <td>{$item['prefix']}{$item['auto_lager']}</td>
        <td>" . Carbon::parse($item['jv_date'])->format('d-m-y') . "</td>
        <td>{$item['ac2']} {$item['Narration']}</td>
...
<tr style=\"background-color:#d9edf7; font-weight:bold;\">
    <td colspan=\"3\" style=\"text-align:center; font-style:italic;\">$num_to_words</td>
...
// Output PDF
$filename = "general_ledger_r_of_{$accountName}_from_{$formattedFromDate}_to_{$formattedToDate}.pdf";
$pdf->Output($filename, 'I');
