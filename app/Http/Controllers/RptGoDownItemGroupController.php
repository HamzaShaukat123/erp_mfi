    ...
    private function stockAllTabulargeneratePDF($groupedByItemName, $groupName, $request)
{
    ...
    $headers = array_merge($headers, $availableGauges);

    $pdf->setTableHeaders($headers);

    // Set Header2 dynamically
    $header2Text = "Generated By: " . auth()->user()->name ?? 'Unknown User'; // Add dynamic text for Header2
    $pdf->setHeader2($header2Text);

    // Call CustomHeader2 explicitly
    $pdf->CustomHeader2();
