    ...
    private function stockAllTabulargeneratePDF($groupedByItemName, $request)
    {
        ...
        foreach ($groupedByItemName as $itemName => $items) {
            ...
            foreach ($headerColumns as $gauge) {
                ...
                // Determine what to display and font size
                if ($value === null) {
                ...
                if ($valueToDisplay === '*' && $value < 0) {
                    $html .= "<td style=\"text-align: center; font-size: {$fontSize}; color: red;\">{$valueToDisplay}</td>";
                } else {
                    // For zero, positive, and other (non-negative) values, display with normal or increased font size
                    $html .= "<td style=\"text-align: center; font-size: {$fontSize};\">{$valueToDisplay}</td>";
                }
            }
