...
// Ensure numColumns is not zero to avoid division by zero
...
foreach ($groupedByItemName as $itemIndex => $items) {
    // Ensure itemIndex is treated as an integer to avoid "A non-numeric value encountered"
    ...
    foreach ($availableGauges as $gauge) {
        ...
        // Ensure the value is numeric and handle null or non-numeric values
        $value = isset($item['opp_bal']) && is_numeric($item['opp_bal']) ? $item['opp_bal'] : 0;
